function [Password, UserName, HostName] = logindlg2(varargin)
% logindlg
% [Password, UserName, HostName] = logindlg(varargin)
%
...
% verification.
% -----------------------------------------------------------------------
% [Password, UserName] = logindlg('enterUserName', true)
%
% Create a user name and password entry dialog box without password
...
% -----------------------------------------------------------------------
% [Password] = logindlg('ValidatePassword', true);
%
...
% -----------------------------------------------------------------------
% [Password, UserName] = logindlg('CheckPasswordLength', true,...
%      'PasswordLengthMin', 5,...
%      'PasswordLengthMax', 8)
%
% -----------------------------------------------------------------------
...
if ~usejava('swing')
   error('logindlg: Java is required for this program to run.');
end
...
ProgramOptionsParser.addParamValue('PasswordLengthMax', 100, @isnumeric);
ProgramOptionsParser.addParamValue('WindowName', 'Login', @ischar);
ProgramOptionsParser.addParamValue('enterHostName', false, @(x) islogical(x) || isnumeric(x));
ProgramOptionsParser.addParamValue('DefaultHostName', '', @ischar);
...
set(handles.edit_Password, ...
    ...
    'Tag', 'edit_Password', ...
    'Units', 'Pixels', ...
    'Position',[PositionLeft 10+OffsetBottom BoxWidth 25]);
...
handles.text_LabelPassword = uicontrol('Parent',handles.figure1, ...
    ...
    'Style','Text', ...
    'Units','Pixels',...
    'Position',[PositionLeft 35+OffsetBottom BoxWidth 25], ...
    'FontSize',10, ...
    'String','Password:',...
    'HorizontalAlignment', 'Left');
...
handles.pushbutton_OK = uicontrol('Parent',handles.figure1, ...
    ...
    'Style','Pushbutton', ...
    'Units','Pixels',...
    'Position',[PositionLeft+BoxWidth+5 10 50 25], ...
    'FontSize',10, ...
    'String','OK',...
...
handles.pushbutton_Cancel = uicontrol('Parent',handles.figure1, ...
    ...
    'Style','Pushbutton', ...
    'Units','Pixels',...
    'Position',[PositionLeft+BoxWidth+50+7 10 70 25], ...
    'FontSize',10, ...
    'String','Cancel',...
    'HorizontalAlignment', 'Center');
...
if ProgramOptions.enterUserName
    ...
    set(handles.edit_UserName, ...
        ...
        'Tag', 'edit_UserName', ...
        'Units', 'Pixels', ...
        'Position',[PositionLeft 68+OffsetBottom 200 25]);
    ...
    handles.text_LabelUserName = uicontrol('Parent',handles.figure1, ...
        ...
        'Style','Text', ...
        'Units','Pixels',...
        'Position',[PositionLeft 95+OffsetBottom 200 25], ...
        'FontSize',10, ...
        'String','User name:',...
        'HorizontalAlignment', 'Left');
...
% Create Host Name Edit Box
if ProgramOptions.enterHostName
    handles.java_HostName = javax.swing.JTextField();
    handles.java_HostName.setFocusable(true);
    [handles.java_HostName, handles.edit_HostName] = javacomponent(handles.java_HostName, [], handles.figure1);
    ...
    set(handles.edit_HostName, ...
        'Parent', handles.figure1, ...
        'Tag', 'edit_HostName', ...
        'Units', 'Pixels', ...
        'Position',[PositionLeft offset_username+68+OffsetBottom 200 25]);
    set(handles.java_HostName, 'Text', ProgramOptions.DefaultHostName);
    drawnow;    % This drawnow is required to allow the focus to work

    handles.text_LabelHostName = uicontrol('Parent',handles.figure1, ...
        'Tag', 'text_LabelHostName', ...
        'Style','Text', ...
        'Units','Pixels',...
        'Position',[PositionLeft offset_username+95 200 25], ...
        'FontSize',10, ...
        'String','Host Name:',...
        'HorizontalAlignment', 'Left');

    %uicontrol(handles.edit_HostName);
    %set(handles.figure1,'CurrentObject',handles.java_HostName)
    handles.java_HostName.requestFocus;     % Get focus
    drawnow;
else
    handles.java_Password.requestFocus;     % Get focus
    drawnow;
...
if ProgramOptions.enterHostName
    set(handles.java_Password, 'NextFocusableComponent', handles.java_HostName);
    if ProgramOptions.enterUserName
        set(handles.java_HostName, 'NextFocusableComponent', handles.java_UserName);
    else
        set(handles.java_HostName, 'NextFocusableComponent', handles.java_Password);
    end
...
if ProgramOptions.enterHostName
    HostName = strtrim(get(handles.java_HostName, 'Text'));
else
    HostName = '';
end;
delete(handles.figure1);
...
if ProgramOptions.enterUserName
    ...
    if isempty(UserName)
        ...
        %disp(strMessage)
        hError = errordlg(strMessage, 'logindlg');
        uiwait(hError);
        return;
    end;
end;
...
if ProgramOptions.ValidatePassword
    ...
    if ~isequal(handles.java_Password.Password, handles.java_PasswordValidate.Password)
        ...
        %disp(strMessage);
        hError=errordlg(strMessage, 'logindlg','modal');
        uiwait(hError);
