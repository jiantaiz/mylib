...
switch command
    ...
    case {'init'}
        ...
        if isfield(opt,'tile_size') && ~isempty(opt.tile_size)
            nii_view.tile_size = opt.tile_size;
        else
            nii_view.tile_size = [1 1 1];            
        end
    ...
    case 'crosshair',
        ...
        if get(nii_view.handles.xhair,'value') == 3		% off
...
if isempty(area)
    area = [0.05 0.05 0.9 0.9];
        area = [0.001 0.001 0.999 0.999];%%SUI
end
...
if usecolorbar
    cbar_area = area;
    cbar_area(1) = area(1) + area(3)*0.96;
    cbar_area(2) = area(2) + area(4)*0.01;
    cbar_area(3) = area(3)*0.02;
    cbar_area(4) = area(4)*0.96;
    area(3) = area(3)*0.95;		% 90% used for main axes
else
    cbar_area = [];
end
...
if nii_view.numscan > 1
    set(handles.contrast, 'min', 0, 'max', nii_view.numscan+1, ...
...
set(fig, 'WindowKeyPressFcn',@WKPFcn);
...
function WKPFcn(varargin)
fh = varargin{1};
keydata = varargin{2};
nii_view = getappdata(gcf,'nii_view');
switch keydata.Character
    case ' '
        if ~isfield(nii_view,'timer') || isempty(nii_view.timer)
            t = timer;
            fig = gcf;
            t.TimerFcn = @(varargin) timer_fcn(varargin{:},fig);
            t.Period = 0.2;
        ...
        elseif strcmp(nii_view.timer.Running , 'on')
            stop( nii_view.timer)
        else
            start( nii_view.timer)
        end
    case {'+','='}
        ...
        if isfield(nii_view,'timer') && ~isempty(nii_view.timer)
            ...
            nii_view.timer.Period= ceil(1000*nii_view.timer.Period  / 1.05)/1000;
            start(nii_view.timer);
        end
    case {'-','_'}%movie slow down
        ...
        if isfield(nii_view,'timer') && ~isempty(nii_view.timer)
            ...
            nii_view.timer.Period=floor(1000*nii_view.timer.Period  * 1.05)/1000;
            start(nii_view.timer);
        end
    case {'g'}
        save_to_gif([]);
...
setscanid = get(nii_view.handles.contrast,'value');

setscanid = setscanid+1;
...
if isempty(nii_view) || ~isfield(nii_view,'handles')
    if verLessThan('matlab', '9.0.1')
        top_ax = axes('position', top_pos);
        front_ax = axes('position', front_pos);
        side_ax = axes('position', side_pos);
    else
        top_ax = axes(fig,'position', top_pos);
        front_ax = axes(fig,'position', front_pos);
        side_ax = axes(fig,'position', side_pos);
    end
...
if isempty(nii_view) || ~isfield(nii_view,'handles') || ~isfield(nii_view.handles,'cbar_axes') || isempty(nii_view.handles.cbar_axes)
    %       cbarminmax_axes = axes(fig,'position', cbar_area);
    %       cbar_axes = axes(fig,'position', cbar_area);
...
set(gcf,'CurrentAxes',cbarminmax_axes)

   plot(cbarminmax_axes,[0 0], cbarminmax, 'w');
   axis tight;
   ...
   ytickb = cbarminmax_axes.YTick;
    ytick=(ylim(2)-ylim(1))*(ytickb-ylimb(1))/(ylimb(2)-ylimb(1))+ylim(1);

%    axes(cbar_axes);
   set(gcf,'CurrentAxes',cbar_axes)


if ~exist('nii_view', 'var')
    nii_view = getappdata(fig, 'nii_view');
end
if isempty(nii_view) || ~isfield(nii_view,'handles')|| ~isfield(nii_view.handles,'cbar_image') || isempty(nii_view.handles.cbar_image)
...
elseif nii_view.nii.hdr.dime.datatype == 511
    ...
    set(nii_view.handles.imval,'Value',real(imgvalue));
...
for ii = 0:n(1)-1
    for jj=0:n(2)-1
        for kk=0:n(3)-1
            cnt=cnt+1;
            x = p_axi(1) + ii/n(1).*nii_view.dims(1);
            x = mod(x-1,nii_view.dims(1))+1;
            y = p_axi(2) + jj/n(2).*nii_view.dims(2);
            y = mod(y-1,nii_view.dims(2))+1;
            p_axi2(cnt,:) = [x,y];
            
            x = p_cor(1) + ii/n(1).*nii_view.dims(1);
            x = mod(x-1,nii_view.dims(1))+1;
            y = p_cor(2) + kk/n(3).*nii_view.dims(3);
            y = mod(y-1,nii_view.dims(3))+1;
            p_cor2(cnt,:) = [x,y];
            
            x = p_sag(1) + jj/n(2).*nii_view.dims(2);
            x = mod(x-1,nii_view.dims(2))+1;
            y = p_sag(2) + kk/n(3).*nii_view.dims(3);
            y = mod(y-1,nii_view.dims(3))+1;
            p_sag2(cnt,:) = [x,y];
        end
    end
...
% p_axi2(:,1) = p_axi(1) + [0: n(1)-1]/n(1).*nii_view.dims(1);
% p_axi2(:,1) = mod(p_axi2(:,1)-1,nii_view.dims(1))+1;
% p_axi2(:,2) = p_axi(2) + [0: n(2)-1]/n(2).*nii_view.dims(2);
% p_axi2(:,2) = mod(p_axi2(:,2)-1,nii_view.dims(2))+1;
% 
% p_cor2(:,1) = p_cor(1) + [0: n(1)-1]/n(1).*nii_view.dims(1);
% p_cor2(:,1) = mod(p_cor2(:,1)-1,nii_view.dims(1))+1;
% p_cor2(:,2) = p_cor(2) + [0: n(3)-1]/n(3).*nii_view.dims(3);
% p_cor2(:,2) = mod(p_cor2(:,2)-1,nii_view.dims(3))+1;
% 
% p_sag2(:,1) = p_sag(1) + [0: n(2)-1]/n(2).*nii_view.dims(2);
% p_sag2(:,1) = mod(p_sag2(:,1)-1,nii_view.dims(2))+1;
% p_sag2(:,2) = p_sag(2) + [0: n(3)-1]/n(3).*nii_view.dims(3);
% p_sag2(:,2) = mod(p_sag2(:,2)-1,nii_view.dims(3))+1;


if get(nii_view.handles.xhair,'Value') == 1 %cursor
    hair_length = 3;
else %crosshair
    hair_length = inf;
end
...
nii_view.axi_xhair = ...
    rri_xhair(p_axi2, nii_view.axi_xhair, nii_view.handles.axial_axes,hair_length);

nii_view.cor_xhair = ...
    rri_xhair(p_cor2, nii_view.cor_xhair, nii_view.handles.coronal_axes,hair_length);

nii_view.sag_xhair = ...
    rri_xhair(p_sag2, nii_view.sag_xhair, nii_view.handles.sagittal_axes,hair_length);
...
get(nii_view.handles.axial_axes,'clim');
