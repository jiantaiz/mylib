...
%         text_toggle=1;
    ...
    case 'w'
        colormap(awave);
        lims = get(gca,'Clim');;
        max_abs_lims = max(abs(lims));
        
         state.clim(:,1) =-max_abs_lims;
         state.clim(:,2) =max_abs_lims;
         set(findobj(fig,'Type','axes'),'Clim',state.clim(1,:));
         set(fig, 'UserData',state);
        return;
    case 'j'
        colormap(jet);
        return;
    case 'o' % change layout
        figname = get(gcf,'name');
        answer = inputdlg('Set layout','',1,{'[1 nan]'});
        answer = eval(['[' answer{:} ']']);
        if ~isempty(answer)
            imdisp(state.I,'size',answer);
...
%         figure;
    ...
    case '5'%auto W/L indivially
        ...
        set(findobj(gcf,'Type','image'),'UserData',[]);
...
%             state.timer.Period=(floor(abs(10000/(1/state.timer.Period-0.5)))+1) /10000; %round to 0.001s
    ...
    case 'x' %orthor_view
        if iscell(state.I)
            ortho_view(cat(3,state.I{:}));
        end
        return;
    case 's' %save current image to worksapce
        im = findobj(gca,'type','Image');
        assignin('base','IMG',im.CData);
        return;
    case 'l' %toggle log scale
        
        im = findobj(gca,'type','Image');
        if strcmp(im.UserData,'log')
            im.CData = 10.^(double(im.CData));
            im.UserData = 'linear';
            clim = optlims(im.CData);
            set(gca,'Clim',clim)
        else
            im.CData = log10(double(im.CData));
            im.UserData = 'log';
            clim = optlims(im.CData);
            set(gca,'Clim',clim)
...
imlineobj = findobj(gca,'Tag','imline');
