function [t_out,wave_out] = plotPdFile(fname, waveGenerators, entryPoint)
...
%       INPUTS:
%        fname : pulse database file name(.pd or .pdx)
...
%        entryPoint : sequence entry point, default 1
%       OUTPUTS:
%        t_out : time in msec
%        wave_out : sequence waveform in instruction amplitudes ( -2^30 ~ 2^30).
...
for num = 1:N_WAVEGEN
    ...
    time = sequence(WGEN,entryPoint).time;
    wave = sequence(WGEN,entryPoint).waveform;%./2^30;
    ...
    if nargout >0
        if isempty(strfind(name,'SSP'))
            ...
            warning('SSP board waveform ignored');
        end
    end
end
